...
import code
class StimuliProcessingMixinNOPE(
    ):
    """
    """
    ...
    def _calculatePulseIntervalTimestamps(
        ...
        iIntervals = np.where(np.diff(iPulses) > 10200)[0] #figure out threshold value 
        ...
        if len(fileList) != intervalTimestamps.shape[0] + 1:
            filtered = filterPulsesWithSmoothing(labjackData[:, 9])
            iPulses = np.where(np.diff(filtered) > 0.5)[0]
            iPulsesFall = np.where(np.diff(filtered) < -0.5)[0] # fall times, should be one for each.
    ...
    def _createMetadataFileList(self, pulseTimestamps, intervalTimestamps
        ...
        print(len(fileList))
        print(intervalTimestamps.shape[0] + 1)
    ...
    def _processDriftingGratingProtocol(
        ...
        # no pulseCountMismatch, whole block is good. Load the whole kaboodle
            ...
            if blockType == 'drifting':
                ...
                eventIndex += blockLength
            elif blockType == 'grayscreen':
                print('grayscreen')
                metadataHolder[eventIndex:eventIndex + blockLength, 0:5] = np.nan
                eventIndex += blockLength
