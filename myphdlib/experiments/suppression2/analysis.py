...
def _parseVisualResponse(neuron, session, window=(-0.2, 0.5), binsize=0.005, probeParsingResults=None, nResamples=50):
    """
    """

    if probeParsingResults is None:
        probeParsingResults = session.parseVisualProbes()

    output = {
        'extrasaccadic': {'ipsi': None, 'contra': None},
        'perisaccadic': {'ipsi': None, 'contra': None}
    }

    #
    for motion in ('ipsi', 'contra'):
        nPerisaccadicTrials = probeParsingResults['perisaccadic'][motion]['timestamps'].size
        nExtrasaccadicTrials = probeParsingResults['extrasaccadic'][motion]['timestamps'].size
        probeOnsetTimestamps = probeParsingResults['perisaccadic'][motion]['timestamps']
        t, M = psth2(
            probeOnsetTimestamps,
            neuron.timestamps,
            window=window,
        ...
        fr = M.mean(0) / binsize
        output['perisaccadic'][motion] = fr

        #
        sample = np.full([nResamples, fr.size], np.nan)
        for iResample in range(nResamples):
            indices = np.random.choice(
                np.arange(0, nExtrasaccadicTrials, 1),
                size=nPerisaccadicTrials
            )
            probeOnsetTimestamps = probeParsingResults['extrasaccadic'][motion]['timestamps'][indices]
            t, M = psth2(
                probeOnsetTimestamps,
                neuron.timestamps,
                window=window,
            ...
            fr = M.mean(0) / binsize
            sample[iResample, :] = fr
        output['extrasaccadic'][motion] = sample.mean(0)
